...
resnet34_model = Sequential()
pretrained_model = ResNet34(include_top=False,input_shape=(224, 224, 3), weights='imagenet',classes=3)
for layer in pretrained_model.layers:
        layer.trainable=False
resnet34_model.add(pretrained_model)
resnet34_model.add(Flatten())
resnet34_model.add(Dense(512, activation='relu'))
resnet34_model.add(Dense(3, activation='softmax'))
...
checkpointer = tf.keras.callbacks.ModelCheckpoint("resnet34_model.h5", verbose=1, save_best_only=True)
callbacks = [
...
train_pred_proba = predict_model(resnet34_model, train_generator, steps=train_steps)
val_pred_proba = predict_model(resnet34_model, validation_generator, steps=val_steps)
