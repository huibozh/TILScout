...
checkpointer = tf.keras.callbacks.ModelCheckpoint('InceptionResNetV2_model.h5', verbose=1, save_best_only=True)
callbacks = [
        tf.keras.callbacks.EarlyStopping(patience=4, monitor='loss'),
        tf.keras.callbacks.TensorBoard(log_dir='logs'),
        checkpointer]

history = InceptionResNetV2_model.fit(
    train_generator,
    epochs=50,
...
######### prediction
"""
InceptionResNetV2_model=keras.models.load_model('InceptionResNetV2_model.h5')
...
def calculate_auc(y_true, y_score, num_classes):
    ...
    auc_scores = []
    for i in range(num_classes):
        if y_true_bin[:, i].sum() > 0:  # Only calculate AUC if there are true samples
